!gfortran  real1d_nhpbec_fft.f90 -I C:\Users\premabrata\vcpkg\installed\x86-windows\include -L C:\Users\premabrata\vcpkg\installed\x86-windows\lib -lfftw3
...
MODULE COMM_DATA
  INTEGER, PARAMETER :: NRUN = 50*10000, LSTORE = 5000, IMSTORE = NRUN / 5
  INTEGER, PARAMETER :: INTERACTION = 100000

  INTEGER, PARAMETER :: NX = 16384, NXX = NX-1, NX2 = NX/2
  REAL (8), PARAMETER :: LX = 163.84D0
...
!  			
  REAL (8), PARAMETER :: NU = 0.01D0, NU2 = NU * NU
  REAL (8), PARAMETER :: G0 = 5.0D0, G01 = 0.01D0, GAM_EFF0 = 0.001D0

!
  REAL (8)    ::X(NX), X2(NX)
  REAL (8)    :: KX(NX), KX2(NX)
  REAL (8)    :: R2(NX)
  COMPLEX (8) :: CP(NX), KIN(NX),V(NX)
...
! Initial Profile
  CP2 = REAL(CP) * REAL(CP) + AIMAG(CP) * AIMAG(CP)
  DO I = 1, NX
    WRITE(50, *) X(I),  CP2(I), RES(I)
  END DO
  WRITE(50, *) X(NX) + DX, CP2(1), RES(1) ! to makeup last point
  CLOSE(50)
! 
...
!
 ...
 ! G=0.0D0
  ...
  OPEN(250, file='POTENTIAL.txt', status='unknown')
  T = 0.0D0
	!GSTP  = G0/INTERACTION
    ...
    DO L = 1, INTERACTION
      ...
      DO I = 1, NX
         !V(I) = (INTERACTION - L) * (1.0D0/INTERACTION) * (X2(I) * NU2 / 2.0D0+CI*X(I)*0.01D0)  
		 V(I) = (X2(I) * NU2 / 2.0D0+CI*X(I)*0.01D0) 
      END DO
	 ...
	 ! G  = G  +  GSTP
      ...
      CALL EFFTW(PLAN1, PLAN2, KIN, CP) 
				...
				OPEN(400, file='repulsive-dyn.txt', status='unknown')
				TI=TI+DT
				IF (MOD(L, LSTORE) == 0) THEN 
				CP2 = REAL(CP) * REAL(CP) + AIMAG(CP) * AIMAG(CP)
				DO I = 1, NX, 4
				WRITE(400, *) X(I), TI, CP2(I)
				END DO
				WRITE(400, *) X(NX) + DX, TI, CP2(1)
				WRITE (400, *)
				WRITE(250, *) X(I), TI, AIMAG(V(I))
				END IF
...
!
      ...
      DO I = 1, NX, 4
        WRITE(15, *) X(I), TI, CP2(I)
      END DO
      WRITE(15, *) X(NX) + DX, TI, CP2(1)
      WRITE (15, *)
...
SUBROUTINE CALCULATE_TRAP()
  USE COMM_DATA, ONLY : NX,CI_
  USE GPE_DATA, ONLY :  NU2, V, X2,X
  IMPLICIT NONE
  INTEGER :: I
  REAL(8)::NH=0.01D0
  
  DO I = 1, NX
      V(I) = NU2 * X2(I)/2.0D0 + CI_*X(I)*NH
    ! V(I) = NU2 * X2(I)**3-3.0D0*NU*X2(I)+CI*			
...
SUBROUTINE EODE(DT, CP) ! Exact solution
  ...
  USE GPE_DATA, ONLY : X,V, G, G1, GAM_EFF
  IMPLICIT NONE
!-------------------------------------------------
  COMPLEX (8), DIMENSION(:), INTENT(INOUT) :: CP
  REAL (8), INTENT(IN) :: DT
