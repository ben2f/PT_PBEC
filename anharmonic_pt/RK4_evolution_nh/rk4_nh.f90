...
function f1(x,y,z,t,gme,g,g1,omega)
	implicit none
	double precision::x,y,z,f1,t,gme,omega,g1,w2,g
	w2=(5.0D0/(2.0D0*sqrt(2.0D0)))
	f1=2.0D0*(gme-z)*x-w2*g1*(x*x)
end function f1
...
function f3(x,y,z,t,gme,g,g1,omega)
	implicit none
	double precision::x,y,z,f3,t,g,omega,g1,gme,d
	d=2.0D0*sqrt(2.0D0)
	f3=-3.0D0*omega+1.0D0/(2.0D0*y*y)+(g*x)/(d*y)-(2.0D0*z*z)-(90*omega*omega*y*y)/8.0D0
end function f3
...
program rk4
	...
	omega=0.1D0
	gme=0.0001D0
	y=1.97848328D0		!update initial values
