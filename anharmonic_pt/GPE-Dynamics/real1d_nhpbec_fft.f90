...
MODULE COMM_DATA
  INTEGER, PARAMETER :: NRUN = 25*10000, LSTORE = 1000, IMSTORE = NRUN / 5
...
!  			
  REAL (8), PARAMETER :: NU = 0.5D0, NU2 = NU * NU,NH=0.3D0
  REAL (8), PARAMETER :: G0 = -10.0D0, G01 = 0.0D0, GAM_EFF0 = 0.0D0
   ...
   !GSTP  = G0/INTERACTION
    ...
    DO L = 1, INTERACTION
      ...
      DO I = 1, NX
		  ...
		  V(I) = (INTERACTION - L) * (1.0D0/INTERACTION) * (NU2*(X2(I)**3)-3.0D0*NU*X2(I)+CI*NH*(X(I)**3)*EXP(-NU*(X(I)**4)/4.0D0))	
...
!---------------------------------------------------------------------------------------------
	  OPEN(17, file='real1d-fin.txt', status='unknown')
      DO I = 1, NX
        WRITE(17, *) X(I), CP2(I)
      END DO
      WRITE(17, *) X(NX) + DX,CP2(1) ! to makeup last point
      CLOSE(17)	  	  
!---------------------------------------------------------------------------------------------- 
!
...
SUBROUTINE CALCULATE_TRAP()
  ...
  USE GPE_DATA, ONLY : NU, NU2, V, X2,X,NH
  IMPLICIT NONE
  INTEGER :: I
  !REAL(8)::NH=0.01D0
  ...
  DO I = 1, NX
	 ...
	 V(I)=NU2*(X2(I)**3)-3.0D0*NU*X2(I)+CI_*NH*(X(I)**3)*EXP(-NU*(X(I)**4)/4.0D0)
